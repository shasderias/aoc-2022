...
func NewPointsFromRange(min, max Point) []Point {
	var points []Point
...
func NewPointSetFromRange(min, max Point) PointSet {
	ps := PointSet{}
	for y := min.Y; y <= max.Y; y++ {
		for x := min.X; x <= max.X; x++ {
			ps.Add(Point{x, y})
...
func (ps PointSet) Remove(p Point) { delete(ps, p) }
...
func (ps PointSet) Has(p Point) bool {
	_, ok := ps[p]
	return ok
}
...
func (ps1 PointSet) Eq(ps2 PointSet) bool {
	if len(ps1) != len(ps2) {
		return false
	}
	for p := range ps1 {
