...
if (treeMap[treeMap.length - 1].length === 0) treeMap.pop(); // remove trailing empty line
...
const testVisible = (treeMap: number[][], maxHeight: number, x: number, y: number) => {
...
const scenicScore = (treeX: number, treeY: number): number => {
    const height = treeMap[treeY][treeX];
    let bViewDist = 0;
    for (let y = treeY + 1; y < yMax; y++) {
        bViewDist++;
        if (treeMap[y][treeX] >= height) break;
    }
    let tViewDist = 0;
    ...
    for (let x = treeX - 1; x >= 0; x--) {
        lViewDist++;
        if (treeMap[treeY][x] >= height) break;
    }
    let rViewDist = 0;
...
for (let coord of visibleSet) {
    ...
    // console.log(`(${x},${y}) = ${s}`);
    if (s > highestScenicScore) highestScenicScore = s;
}
